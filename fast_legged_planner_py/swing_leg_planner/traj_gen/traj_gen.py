...
LastEditTime: 2023-11-21 12:36:16
...
class HermiteSpline(SplineBase):
    ...
    def evaluate(self, t, eval_mode="pos"):
        ...
        if index == self.n-2:  # Special case
            return cubic_evaluate(self.knots[-4:], 1, self.para_mat, eval_mode)
        return cubic_evaluate(self.knots[index:index+4], t-int(t), self.para_mat, eval_mode)
    ...
    def insert(self, t_list: list):
        ...
        for t in t_list:
            ...
            knots = np.insert(knots, 2*int(t)+2, P_new1[2:], axis=0)  # Knot insert
