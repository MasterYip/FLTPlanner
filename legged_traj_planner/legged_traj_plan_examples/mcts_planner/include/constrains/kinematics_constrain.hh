...
namespace Robot_State_Transition
{
    ...
    // 质心约束足端,返回全部腿约束不等式; 增加腿部碰撞的约束
    void get_kinematics_con_cog_foot_selfColision(const MDT::RobotState &state, MatrixXX *A_output, VectorX *b_output, bool isReduce, const Parameters &params);

    // 质心约束足端,返回单腿约束不等式
    std::pair<MatrixXX, VectorX> get_oneLegKinematics_con_cog_foot(const MDT::Pose &base_pose, 
        int legN, bool isReduce, const Parameters &params);

    // 足端约束机体的质心
    std::pair<MatrixXX, VectorX> get_kinematics_con_foot_cog(const MDT::Pose &base_pose,const std::vector<int> &support_leg, const MatrixX3 &contactPoints, 
    const MatrixX3 &contactNormals, bool isReduce, const Parameters &params);
