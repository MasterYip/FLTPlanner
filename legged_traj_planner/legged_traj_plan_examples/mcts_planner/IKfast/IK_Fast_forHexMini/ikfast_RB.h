...
#ifndef IKFAST_HEADER_COMMON_RB
#define IKFAST_HEADER_COMMON_RB
...
struct AlignedSolution {
    ...
    template <typename T>
    void SetIkSolution(ikfast_RB::IkSolution<T>& solnobj, const T v[]) {
        const uint32_t dof = solnobj.GetDOF();
        // assert(freejoint < dof && mimicjoint < dof);
        ikfast_RB::IkSingleDOFSolutionBase<T>& freejointsoln = solnobj[freejoint],
...
{
    IndicesCompare(
        const std::vector<ikfast_RB::IkSolution<T> >& vecsols,
    ...
    {
        const ikfast_RB::IkSolution<T> &sola = _vecsols[inda],
                                    &solb = _vecsols[indb];
    ...
    const std::vector<ikfast_RB::IkSolution<T> >& _vecsols;
    const std::array<uint32_t, N>& _jointorder;
};

template <typename T, long unsigned int N>
void DeriveSolutionIndices(std::vector<ikfast_RB::IkSolution<T> >& vecsols,
                           const std::array<uint32_t, N>& jointorder) {
    if( vecsols.empty() ) {
        return;
    }
    ...
    // assert(N <= nallvars);
    // for(ikfast_RB::IkSolution<T>& vecsol : vecsols) {
    ...
    for(uint32_t si = 1; si < numsolns; si++) {
        ...
        const ikfast_RB::IkSolution<T> &prevsol = vecsols[vindices[si-1]];
        ikfast_RB::IkSolution<T> &cursol = vecsols[vindices[si]];
    ...
    const ikfast_RB::IkSolution<T> &cursol = vecsols[vindices[numsolns-1]];
    for (uint32_t varindex = 0; varindex < N; varindex++) {
        const uint32_t jointindex = jointorder[varindex],
    ...
    for (size_t a = 0; a < vecsols.size(); ++a) {
        ikfast_RB::IkSolution<T>& vecsol = vecsols[a];
...
#define IKFAST_API
#endif
#define IKFAST_NAMESPACE_RB


#ifdef IKFAST_NAMESPACE_RB
namespace IKFAST_NAMESPACE_RB_SPACE {
#endif
 ...
 */
IKFAST_API bool ComputeIk(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, ikfast_RB::IkSolutionListBase<IkReal>& solutions);

/** \brief Similar to ComputeIk except takes OpenRAVE boost::shared_ptr<RobotBase::Manipulator>*
 */
IKFAST_API bool ComputeIk2(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, ikfast_RB::IkSolutionListBase<IkReal>& solutions, void* pOpenRAVEManip);
