...
std::pair<bool, Eigen::Matrix<double, Eigen::Dynamic, 1>> fixGu_Frame_IK(Eigen::Matrix<double, 6, 3> initFeetPosition_FixGu)
{
    ...
    for(int legN =0; legN<6; legN++)
    {
        ...
        if(solret.size() != 1)
        {
            ...
            std::cout << "\033[31m IKFault occurred!\033[0m" << std::endl;
...
//指定某条腿
Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> getFootP_i_fixGu_from_FeetP_W(float x, float y, float z,float roll, float pitch, float yaw,  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> feetPosition_W, int legIndex)
{
    Eigen::Isometry3d T_world_base = getT_W_B(x, y, z, roll, pitch, yaw);
    Eigen::Isometry3d T_B_FixGu[6];
    get_T_B_FixGu(T_B_FixGu);
    ...
    Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> feetPosition_FixGu(1, 3);

    Eigen::Matrix<double, 3, 1> p_W;
    p_W << feetPosition_W(legIndex,0), feetPosition_W(legIndex,1), feetPosition_W(legIndex,2);

    Eigen::Matrix<double, 3, 1> p_B = T_world_base.inverse()*p_W; // 足端坐标在body坐标系下

    // 将足端坐标转换到FixGu坐标系下
    Eigen::Matrix<double, 3, 1> p_FixGu = T_B_FixGu[legIndex].inverse()*p_B; // 足端坐标在FixGu坐标系下
    // std::cout << "p_FixGu: " << p_FixGu.transpose() << std::endl;
    feetPosition_FixGu.row(0) = p_FixGu;
...
// 输入是每条腿在fixgu坐标系下的坐标, 求解
std::pair<bool, Eigen::Matrix<double, Eigen::Dynamic, 1>> fixGu_Frame_IK_oneLeg(Eigen::Matrix<double, 1, 3> initFeetPosition_FixGu, int legIndex)
{
    ...
    for(int legN =0; legN<6; legN++)
    {
        ...
        footP.push_back(initFeetPosition_FixGu(0,0));
        footP.push_back(initFeetPosition_FixGu(0,1));
        footP.push_back(initFeetPosition_FixGu(0,2));
...
// 求解全身的逆运动学
std::pair<bool, Eigen::Matrix<double, Eigen::Dynamic, 1>> leg_IK(float x_W, float y_W, float z_W,float roll, float pitch, float yaw,  Eigen::Matrix<double, Eigen::Dynamic, Eigen::Dynamic> feetPosition_W, int legIndex)
{
    auto inputGu = getFootP_i_fixGu_from_FeetP_W(x_W, y_W, z_W, roll, pitch, yaw, feetPosition_W, legIndex);
    auto result = fixGu_Frame_IK_oneLeg(inputGu, legIndex);
    return result;
}
