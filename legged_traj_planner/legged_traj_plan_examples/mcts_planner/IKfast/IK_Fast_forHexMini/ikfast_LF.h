...
#ifndef IKFAST_HEADER_COMMON_LF
#define IKFAST_HEADER_COMMON_LF
...
struct AlignedSolution {
    ...
    template <typename T>
    void SetIkSolution(ikfast_LF::IkSolution<T>& solnobj, const T v[]) {
        const uint32_t dof = solnobj.GetDOF();
        // assert(freejoint < dof && mimicjoint < dof);
        ikfast_LF::IkSingleDOFSolutionBase<T>& freejointsoln = solnobj[freejoint],
...
{
    IndicesCompare(
        const std::vector<ikfast_LF::IkSolution<T> >& vecsols,
    ...
    {
        const ikfast_LF::IkSolution<T> &sola = _vecsols[inda],
                                    &solb = _vecsols[indb];
    ...
    const std::vector<ikfast_LF::IkSolution<T> >& _vecsols;
    const std::array<uint32_t, N>& _jointorder;
};

template <typename T, long unsigned int N>
void DeriveSolutionIndices(std::vector<ikfast_LF::IkSolution<T> >& vecsols,
                           const std::array<uint32_t, N>& jointorder) {
    if( vecsols.empty() ) {
        return;
    }
    ...
    // assert(N <= nallvars);
    // for(ikfast_LF::IkSolution<T>& vecsol : vecsols) {
    ...
    for(uint32_t si = 1; si < numsolns; si++) {
        ...
        const ikfast_LF::IkSolution<T> &prevsol = vecsols[vindices[si-1]];
        ikfast_LF::IkSolution<T> &cursol = vecsols[vindices[si]];
    ...
    const ikfast_LF::IkSolution<T> &cursol = vecsols[vindices[numsolns-1]];
    for (uint32_t varindex = 0; varindex < N; varindex++) {
        const uint32_t jointindex = jointorder[varindex],
    ...
    for (size_t a = 0; a < vecsols.size(); ++a) {
        ikfast_LF::IkSolution<T>& vecsol = vecsols[a];
...
#define IKFAST_NAMESPACE_LF

#ifdef IKFAST_NAMESPACE_LF
namespace IKFAST_NAMESPACE_LF_SPACE {
#endif
 ...
 */
IKFAST_API bool ComputeIk(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, ikfast_LF::IkSolutionListBase<IkReal>& solutions);

/** \brief Similar to ComputeIk except takes OpenRAVE boost::shared_ptr<RobotBase::Manipulator>*
 */
IKFAST_API bool ComputeIk2(const IkReal* eetrans, const IkReal* eerot, const IkReal* pfree, ikfast_LF::IkSolutionListBase<IkReal>& solutions, void* pOpenRAVEManip);
