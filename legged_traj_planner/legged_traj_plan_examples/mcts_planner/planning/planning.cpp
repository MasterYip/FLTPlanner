...
namespace PLANNING
{
    ...
    float getReduceKinematicMargin(const MDT::Pose &robotPoseW, const MDT::POINT &footW,
                                   const float &forwardDirection_W, const int &legNum_, const Parameters &params)
    // (const int &legNum_, const hit_spider::hexapod_Base_Pose &robotPose, const geometry_msgs::Point &footW, const geometry_msgs::Point &direction_)
    {
        ...
        // MatrixXX A_output[6];
        // VectorX b_output[6];
        auto Ab = Robot_State_Transition::get_oneLegKinematics_con_cog_foot(robotPoseW, legNum_, true, params);
    ...
    bool getStabilityMargin(MDT::POINT RobotCenter, std::vector<MDT::POINT> Pnt, int n, float *margin_)
    {
        ...
        if (!GEOMETRY::InPolygon(RobotCenter, n, Pnt))
        {
            return false;
        }
        for (int i = 0; i < n - 1; i++)
     ...
     * \return 返回可行的支撑状态集
     */
    std::vector<MDT::Vector6b> findAvailableSupportStates(const MDT::RobotState &hexapodState, const Parameters &params)
    {
        // 计算fault leg 数目
        int faultLegNum = 0;
        for (int i = 0; i < 6; i++)
        ...
        if (faultLegNum > 0)
        {
            for (int i = 0; i < HexapodParameter::initialSupportList.size(); i++)
            {
                ...
                for (int j = 0; j < 6; j++)
                {
                    if (hexapodState.faultStateToNow[j] == hexapodState.faultFlag() && HexapodParameter::initialSupportList[i][j] == hexapodState.supportFlag())
                    {
                        tmpNum_++;
                    }
                }
        ...
        for (int i = 0; i < tmpSupportList2.size(); i++)
        {
            ...
            {
                if (params.GaitMode == 1)
                {
                    // 去掉前后相邻俩腿同时摆动的步态
                    // 1,2
                    if (tmpSupportList2[i](0) == MDT::SWING_FLAG && tmpSupportList2[i](1) == MDT::SWING_FLAG)
                    {
                        continue;
                    }
                    // 1,4
                    if (tmpSupportList2[i](0) == MDT::SWING_FLAG && tmpSupportList2[i](3) == MDT::SWING_FLAG)
                    {
                        continue;
                    }
                    // 2,3
                    if (tmpSupportList2[i](1) == MDT::SWING_FLAG && tmpSupportList2[i](2) == MDT::SWING_FLAG)
                    {
                        continue;
                    }
                    // 3,6
                    if (tmpSupportList2[i](2) == MDT::SWING_FLAG && tmpSupportList2[i](5) == MDT::SWING_FLAG)
                    {
                        continue;
                    }
                    // 5,6
                    if (tmpSupportList2[i](4) == MDT::SWING_FLAG && tmpSupportList2[i](5) == MDT::SWING_FLAG)
                    {
                        continue;
                    }
                    // 4,5
                    if (tmpSupportList2[i](3) == MDT::SWING_FLAG && tmpSupportList2[i](4) == MDT::SWING_FLAG)
                    {
                        continue;
                    }
                }
                else if (params.GaitMode == 2) // 对角步态
                {
                    if (tmpSupportList2[i](0) == MDT::SWING_FLAG && tmpSupportList2[i](1) == MDT::SUPPORT_FLAG && tmpSupportList2[i](2) == MDT::SWING_FLAG && tmpSupportList2[i](3) == MDT::SUPPORT_FLAG && tmpSupportList2[i](4) == MDT::SWING_FLAG && tmpSupportList2[i](5) == MDT::SUPPORT_FLAG)
                    {
                        availableSupportList.push_back(tmpSupportList2[i]);
                    }
                    else if (tmpSupportList2[i](0) == MDT::SUPPORT_FLAG && tmpSupportList2[i](1) == MDT::SWING_FLAG && tmpSupportList2[i](2) == MDT::SUPPORT_FLAG && tmpSupportList2[i](3) == MDT::SWING_FLAG && tmpSupportList2[i](4) == MDT::SUPPORT_FLAG && tmpSupportList2[i](5) == MDT::SWING_FLAG)
                    {
                        availableSupportList.push_back(tmpSupportList2[i]);
                    }
                    continue;
    ...
    std::vector<float> findAvailableStepLength(const MDT::RobotState &hexapodState,
                                               const std::vector<MDT::Vector6b> &availableSupportList, const Parameters &params)
    {
        ...
        for (int i = 0; i < 6; i++)
        {
            reduxceKinematicMargin_[i] = PLANNING::getReduceKinematicMargin(hexapodState.pose,
                                                                            hexapodState.feetPosition[i], hexapodState.moveDirection, i, params);
        }
    ...
    float getMaximumTransitionLength_considerJntTorqueConstraint(const MDT::RobotState &hexapodState, const std::vector<int> &supportLegNumList,
                                                                 const MatrixX3 &contactPoints, const Eigen::Vector3f &forwardDir, float stepLength, const Parameters &params)
    {
        ...
        for (auto pose_ : dicresized_pose)
        {
            ...
            if (!Robot_State_Transition::is_meet_dynamic_con2(Vector3(pose_.x, pose_.y, pose_.z), supportLegNumList, contactPoints, joints, params.HexMini_PinoModel))
            {
                break;
            }
    ...
    bool supportLegCollisionCheck(const MDT::RobotState &hexapodState, const std::vector<int> &supportLegNumList,
                                  const Eigen::Vector3f &forwardDir, float stepLength, Parameters &params)
    {
        Eigen::Vector3f direction = forwardDir;
        Eigen::Vector3f start = {hexapodState.pose.x, hexapodState.pose.y, hexapodState.pose.z};
        direction.normalize(); // 归一化!!
        ...
        for (auto pose_ : dicresized_pose)
        {
            ...
            for (int i = 0; i < supportLegNumList.size(); i++)
            {
                ...
                auto feetP_Body = params.HexMini_PinoModel_Data.oMf[params.HexMini_PinoModel.getFrameId(params.FootName[supportLegNumList[i] - 1])].translation();
                auto shankP_Body = params.HexMini_PinoModel_Data.oMf[params.HexMini_PinoModel.getFrameId(params.ShankName[supportLegNumList[i] - 1])].translation();
                auto midP_Body = (shankP_Body + feetP_Body) / 2;
                auto thighP_Body = params.HexMini_PinoModel_Data.oMf[params.HexMini_PinoModel.getFrameId(params.ThighName[supportLegNumList[i] - 1])].translation();
                ...
                {
                    if (!COLLISION_CHECK::isCollision_onePoint(keyPnts[i], params.mapData))
                    {
    ...
    std::pair<Vector3, Vector3> calculateForwardVector3d(const MDT::RobotState &hexapodState, const Parameters &params)
    {
        ...
        for (int i = 0; i < 19; i++)
        {
            for (int j = 0; j < 10; j++)
            {
                ...
                tmpP_ << hexapodState.pose.x - 0.6 + 0.08 * (i), hexapodState.pose.y - 0.5 + 0.1 * (j + 1);
                grid_map::Index it11;
                params.mapData.getIndex(tmpP_, it11);
                if (params.mapData.isValid(it11, params.elevationLayerName))
                {
                    float z = params.mapData.at(params.elevationLayerName, it11);
                    pnts.push_back(Vector3(tmpP_.x(), tmpP_.y(), z));
                    // printf("x: %f, y: %f, z: %f \t", tmpP_.x(), tmpP_.y(), mapData.at(params.elevationLayerName,it11));
                }
            }
        }
    ...
    double pointToSegmentDistance(const Eigen::Vector2d &P, const Eigen::Vector2d &A, const Eigen::Vector2d &B)
    {
        ...
        double distance = std::abs(cross) / AB.norm();
        return distance;
    }

    // 计算点到凸包边缘的最小距离
    double minimumDistanceToConvexHull(const Eigen::Vector2d &point, const Eigen::MatrixXd &A, const Eigen::VectorXd &B)
    {
        ...
        {
            Eigen::Vector2d vertex1 = A.row(i);
            ...
            double distance = pointToSegmentDistance(point, vertex1, vertex2);
    ...
    // 计算点到平面的距离
    double pointToPlaneDistance3D(const Eigen::Vector3d &P, const Eigen::Vector3d &normal, const Eigen::Vector3d &pointOnPlane)
    {
        double distance = std::abs(normal.dot(P - pointOnPlane)) / normal.norm();
        return distance;
    }

    // 计算点到三维凸包面的最小距离
    double minimumDistanceToConvexHull3D(const Eigen::Vector3d &point, const Eigen::MatrixXd &A, const Eigen::VectorXd &B)
    {
        double min_distance = std::numeric_limits<double>::infinity();
        int num_planes = A.rows();

        // 遍历所有面
        for (int i = 0; i < num_planes; ++i)
        {
            Eigen::Vector3d normal = A.row(i);
            double d = B(i);
            Eigen::Vector3d pointOnPlane = normal * (d / normal.squaredNorm());

            double distance = pointToPlaneDistance3D(point, normal, pointOnPlane);
            if (distance < min_distance)
            {
                min_distance = distance;
            }
    ...
    std::pair<float, float> getTargetRollAndPitch(const MDT::RobotState hexapodState, const Parameters &params)
    {
        // 根据落足点拟合的平面，计算沿着平面的三维方向向量 和 平面法向量
        auto results_fitPlane = PLANNING::calculateForwardVector3d(hexapodState, params);
    ...
    std::pair<std::vector<MDT::Vector6b>, std::vector<MDT::POINT>> getSupportListAndStepL_underConstrains(const MDT::RobotState hexapodState, const Parameters &params, bool isPrint = false)
    {

        std::vector<MDT::RobotState> resultLists;
        std::vector<MDT::POINT> maxMoving;

        std::vector<MDT::Vector6b> availableSupportState_ = PLANNING::findAvailableSupportStates(hexapodState, params);
        ...
        int constrainSkipCount[10];
        for (int i = 0; i < 10; i++)
        {
            constrainSkipCount[i] = 0;
        }
        for (int i = 0; i < availableSupportState_.size(); ++i)
        {
            ...
            if (params.IsMaxForceConstraint)
            {
                // std::cout << "contact_maxmumF:" << contact_maxmumF << std::endl;

                if (!Robot_State_Transition::comput_friction_region_considerMaxF(contact_Points, contact_normals, contact_maxmumF, 0.2, params.Mass, CWC_Vertices))
                {
                    constrainSkipCount[0]++;
                    continue;
                }
            }
            else
            {
                if (!Robot_State_Transition::comput_friction_region(contact_Points, contact_normals, 0.2, params.Mass, CWC_Vertices))
                {
                    constrainSkipCount[1]++;
                    continue;
                }
            }
            ...
            {
                constrainSkipCount[3]++;
                continue; // 质心位置不在cwc内部
            }
            // float COG_x1 = -0.09;
            // float COG_x2 = 0.00;

            // if (hexapodState.pose.x > 4.5)
            // {
            //     COG_x1 = -0.00;
            //     COG_x2 = 0.09;
            // }

            // Eigen::Vector3d cowTran_B(COG_x1, 0, 0);
            // Eigen::Vector3d cowTran_W = hexapodState.pose.getT_W_B() * cowTran_B;
            // VectorX is_in_cwc2 = Ab_cwc.first * point_Planar(cowTran_W.x(), cowTran_W.y());

            // if (!((is_in_cwc2.array() <= Ab_cwc.second.array()).all()))
            // {
            //     constrainSkipCount[4]++;
            //     continue; // 质心位置不在cwc内部
            // }

            // Eigen::Vector3d cowTran_B_foward(COG_x2, 0, 0);
            // Eigen::Vector3d cowTran_W_foward = hexapodState.pose.getT_W_B() * cowTran_B_foward;
            // VectorX is_in_cwc_foward = Ab_cwc.first * point_Planar(cowTran_W_foward.x(), cowTran_W_foward.y());

            // if (!((is_in_cwc_foward.array() <= Ab_cwc.second.array()).all()))
            // {
            //     constrainSkipCount[5]++;
            //     continue; // 质心位置不在cwc内部
            // }

            double min_distance = minimumDistanceToConvexHull(point_Planar(hexapodState.pose.x, hexapodState.pose.y), Ab_cwc.first, Ab_cwc.second);

            if (min_distance < params.staticStabilityMarginThreshold)
            {
                constrainSkipCount[6]++;
                continue; // 稳定裕度
            }
            ...
            // if(dis_cwc < params.staticStabilityMarginThreshold) continue;  // 最小前进步长

            // 2.当前支撑腿对质心的运动约束，求解最大前进步长*****************************************************************************
            auto feet_con_cog_Ab = Robot_State_Transition::get_kinematics_con_foot_cog(hexapodState.pose, support_leg, contact_Points, contact_normals, true, params);

            if (!Robot_State_Transition::isInConvex(feet_con_cog_Ab.first, feet_con_cog_Ab.second, COG_W))
            {
                constrainSkipCount[7]++;
            ...
            // if (params.IsToruqeLimitConstraint)
            // {
            ...
            // if(length_ < params.kinematicMargin)
            // {
            //     constrainSkipCount[8]++;
            //     continue;
            // }
            // else length_ -= params.kinematicMargin;

            // Vector3 projectionUpZ(0, 0, 1);

            // float stepLength_ = length_ * 0.8f;
            // COG_W.x() = hexapodState.pose.x + stepLength_ * cos(hexapodState.moveDirection);
            // COG_W.y() = hexapodState.pose.y + stepLength_ * sin(hexapodState.moveDirection);

            // auto intersection2_ = Robot_State_Transition::findIntersection(feet_con_cog_Ab.first, feet_con_cog_Ab.second, COG_W,
            //                                                                projectionUpZ);
            // double maxZ_dis = intersection2_(2) - hexapodState.pose.z;

            // MDT::POINT maxMovingPoint = {length_, pitch, maxZ_dis};
        ...
        if (isPrint)
        {
            std::cout << "there are " << availableSupportState_.size() << " initial available support states" << std::endl;
            int countALl = availableSupportState_.size();
            for (int i = 0; i < 10; i++)
            {
                countALl -= constrainSkipCount[i];
                printf("constrainSkipCount[%d]: %d\n", i, constrainSkipCount[i]);
            }
            std::cout << "there are " << countALl << " final states successfully!" << std::endl;
        }
                                                                                                    ...
                                                                                                    const Parameters &params)
    {
        ...
        for (int i = 0; i < availableSupportState_.size(); ++i)
        {
            ...
            auto yawAngleRange = getMaxYawRotationAngle(testState, params);
    ...
    //     {
    //         if (mapData.isValid(*it, params.elevationLayerName))
    //         {
    //             ++num;
    //         }

    //         // mapData.getPosition(*it, position);
    //         // if (mapData.at(params.elevationLayerName, *it) == 0)
    //         // {
    //         //     ++num;
    //         // }
    //     }
    ...
    //             contactDates.normalVector.row(row_index) << mapData.at(params.normalVector_x_Name, *it), mapData.at(params.normalVector_y_Name, *it), mapData.at(params.normalVector_z_Name, *it);
    //             contactDates.position.row(row_index) << tmpP_.x(), tmpP_.y(), mapData.at(params.elevationLayerName, *it);
    ...
    ContactsInfos get_now_Feasible_foot_position(const MDT::Pose &pose, const Parameters &params)
    {
        ...
        int row_index = 0;
        std::vector<Eigen::Vector3d> normalVector_;
        std::vector<Eigen::Vector3d> position_;

        for (grid_map::CircleIterator it(params.mapData, center, radius); !it.isPastEnd(); ++it)
        {
            // grid_map::Position position;
            // mapData.getPosition(*it, position);
            if (params.mapData.isValid(*it, params.elevationLayerName))
            {
                grid_map::Position tmpP_;
                params.mapData.getPosition(*it, tmpP_);
                normalVector_.push_back(Eigen::Vector3d(0, 0, 1));
                position_.push_back(Eigen::Vector3d(tmpP_.x(), tmpP_.y(), params.mapData.at(params.elevationLayerName, *it)));
        ...
        contactDates.position.resize(row_index, 3);
        contactDates.normalVector.resize(row_index, 3);
        contactDates.frcitionMu.resize(row_index);
        contactDates.maxNormalF.resize(row_index);

        for (size_t i = 0; i < position_.size(); ++i)
        {
            contactDates.position.row(i) << position_[i].x(), position_[i].y(), position_[i].z();
            contactDates.normalVector.row(i) << normalVector_[i].x(), normalVector_[i].y(), normalVector_[i].z();
            contactDates.maxNormalF(i) = 100;
        }
    ...
    MDT::AvailableContactsInfo getAvailableFootholds(const MDT::RobotState &hexapodState, const Parameters &params)
    {
        ...
        auto result = get_now_Feasible_foot_position(hexapodState.pose, params);

        auto map_feasible_position = result.position.transpose();
        ...
        // base约束foot
        Robot_State_Transition::get_kinematics_con_cog_foot_selfColision(hexapodState, A_output, b_output, true, params);

        // std::cout << map_feasible_position.rows() << std::endl;
        // std::cout << map_feasible_position.cols() << std::endl;
        ...
        for (int i = 0; i < 6; ++i)
        {
            if (hexapodState.gaitToNow[i] == MDT::SWING_FLAG)
            {
                ...
                MatrixXX temp = A * map_feasible_position;
                for (int j = 0; j < temp.cols(); ++j)
                {

                    if ((temp.col(j).array() <= b.array()).all())
                    {
                        // std::cout << map_feasible_position.col(j) << std::endl;
                        availableContacts.position.leg[i].push_back(map_feasible_position.col(j));
    ...
    bool swingLegFoot_position_Expert_forSim2(MDT::RobotState &hexapodState, const Parameters &params)
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            if (hexapodState.gaitToNow[i] == MDT::SWING_FLAG)
            {
                Eigen::Vector3d nF = params.norminalFoothold_B[i];
                ...
                if (params.mapData.isValid(it11, params.elevationLayerName))
                {
                    default_p0.z() = params.mapData.at(params.elevationLayerName, it11);
                    feasible_positions.push_back(default_p0);
                }

                tmpP_ << default_p1.x(), default_p1.y();
                grid_map::Index it12;
                params.mapData.getIndex(tmpP_, it12);
                if (params.mapData.isValid(it12, params.elevationLayerName))
                {
                    default_p1.z() = params.mapData.at(params.elevationLayerName, it12);
                    feasible_positions.push_back(default_p1);
                }

                tmpP_ << default_p2.x(), default_p2.y();
                grid_map::Index it13;
                params.mapData.getIndex(tmpP_, it13);
                if (params.mapData.isValid(it13, params.elevationLayerName))
                {
                    default_p2.z() = params.mapData.at(params.elevationLayerName, it13);
                ...
                else
                {
                    ...
                    Eigen::Vector3d dafult_position = params.norminalFoothold_B[i];
    ...
    MDT::RobotState getNextMCTSstateRotate_forSim(const MDT::RobotState rState, const Parameters &params)
    {
        ...
        auto result = getSupportList_Rotate_underConstrains(rState, params);
        ...
        PLANNING::swingLegFoot_position_Expert_forSim2(hexapodState, params);
    ...
    MDT::RobotState getNextMCTSstate_forSim(const MDT::RobotState rState, const Parameters &params)
    {
        ...
        std::vector<MDT::RobotState> resultLists;
        std::vector<MDT::Vector6b> availableSupportState_ = PLANNING::findAvailableSupportStates(rState, params);
        ...
        std::vector<float> maxStepLengthForSupportState = findAvailableStepLength(rState, availableSupportState_, params);
        ...
        hexapodState.pose.z = averageFootholdsHeight + params.norminalTrunkHeight;

        // findAvailableFootholdsCombination
        // findRandomFootholdsCombination
        if (PLANNING::swingLegFoot_position_Expert_forSim2(hexapodState, params))
        {
            return hexapodState;
        }
    ...
    {
        auto resultF = getAvailableFootholds(rState, params);
        ...
        for (int i = 0; i < 6; ++i)
        {
            if (rState.gaitToNow[i] == MDT::SWING_FLAG)
            {
                if (feasible_positions.leg[i].size() != 0)
                {
                    ...
                    if (params.foothold_select_type == "norminal_Foothold_addL")
                    {
                        // 寻找最靠近默认位置的落足点
                        Eigen::Vector3d nF = params.norminalFoothold_B[i];
                        ...
                        {
                            auto tempP = feasible_positions.leg[i][j];
                            // tempP.z() = 0;
                            // default_p2.z() = 0;
                            dis2(j) = (tempP - default_p2).norm();
                        }
                        ...
                        // float dis_ = sqrt(
                        // (rState.feetPosition[i].x() - hexapodState3.feetPosition[i].x())*
                        // (rState.feetPosition[i].x() - hexapodState3.feetPosition[i].x())
                        // +
                        // (rState.feetPosition[i].y() - hexapodState3.feetPosition[i].y())*
                        // (rState.feetPosition[i].y() - hexapodState3.feetPosition[i].y())
                        // )
                        // if(dis < 0.03f)
                        // {
                    ...
                    else if (params.foothold_select_type == "average_addL")
                    {
                        ...
                        x_average /= feasible_positions.leg[i].size();
                        y_average /= feasible_positions.leg[i].size();
                        z_average /= feasible_positions.leg[i].size();

                        Vector3 average_p(x_average + addL, y_average, z_average);
                    ...
                    if (params.IsTest)
                    {
                        if (i < 3)
                        {
                            hexapodState3.maxNormalForce[i] = 80;
                        }
                        else
                        {
                            hexapodState3.maxNormalForce[i] = 500;
                        }
                    }
                ...
                else
                {
                    ...
                    hexapodState3.feetPosition[i].x() = dafult_position.x();
                    hexapodState3.feetPosition[i].y() = dafult_position.y();
                    hexapodState3.feetPosition[i].z() = dafult_position.z();
    ...
    MDT::VectorList swingLegFoot_position_visualize(const MDT::RobotState &rState, const Parameters &params)
    {
        ...
        for (int i = 0; i < 6; ++i)
        {
            ...
            hexapodState3.feetPosition[i] = default_p;
        ...
        for (int i = 0; i < 6; ++i)
        {
            if (rState.gaitToNow[i] == MDT::SWING_FLAG)
            {
                if (feasible_positions.leg[i].size() != 0)
                {
                    ...
                    if (!COLLISION_CHECK::isSwingLegCollision(startP, endP, params.mapData, i, bodyPositionBegin, bodyPositionEnd, bodyPoseBegin, bodyPoseEnd))
                    {
                        flag1 = false;
                    }
    ...
    // return to available state planner
    MDT::RobotState recoverPlanner(const MDT::RobotState currentState, const Parameters &params)
    {

        // 寻找不再工作空间内的腿，或者最靠近工作空间边缘的腿
        MDT::Vector6b supportState;
        supportState << MDT::SUPPORT_FLAG, MDT::SUPPORT_FLAG, MDT::SUPPORT_FLAG, MDT::SUPPORT_FLAG, MDT::SUPPORT_FLAG, MDT::SUPPORT_FLAG;

        std::vector<std::pair<float, int>> kinematicReduceDistance;
        static int lastLegIndex = -1;
        ...
        for (int legIndex = 0; legIndex < 6; legIndex++)
        {
            ...
            auto Ab = Robot_State_Transition::get_oneLegKinematics_con_cog_foot(currentState.pose, legIndex, true, params);
            if (!Robot_State_Transition::isInConvex(Ab.first, Ab.second, footW))
            {
                supportState[legIndex] = MDT::SWING_FLAG;
                break;
            }

            // 计算腿的kinematicReduceDistance
            float inverseDirection = currentState.moveDirection + _PI_;
            MDT::POINT n_ = {cos(inverseDirection), sin(inverseDirection), 0};
            auto intersection_ = Robot_State_Transition::findIntersection(Ab.first, Ab.second, footW, n_);
            float reducedMargin = (intersection_ - footW).norm();
            kinematicReduceDistance.push_back(std::make_pair(reducedMargin, legIndex));

            printf("legIndex: %d, reducedMargin: %f\n", legIndex, reducedMargin);
        }

        if (kinematicReduceDistance.size() == 6)
        {
            // 找到 kinematicReduceDistance 中最小的元素
            auto minElement = std::min_element(kinematicReduceDistance.begin(), kinematicReduceDistance.end(),
            ...
            // 防止连续两次选择同一个腿
            if (minElement->second == lastLegIndex)
            {
                kinematicReduceDistance.erase(minElement);
                minElement = std::min_element(kinematicReduceDistance.begin(), kinematicReduceDistance.end(),
                                              [](const std::pair<float, int> &a, const std::pair<float, int> &b)
                                              {
                                                  return a.first < b.first;
                                              });
            ...
            // 根据 kinematicReduceDistance 的最小值设定对应的 supportState 为 SWING_FLAG
            if (minElement != kinematicReduceDistance.end())
            {
                supportState[minElement->second] = MDT::SWING_FLAG;
            }
            lastLegIndex = minElement->second;
        }

        MDT::RobotState hexapodState = currentState;

        hexapodState.gaitToNow = supportState;

        std::vector<MDT::RobotState> resultLists = swingLegFoot_position_lists(hexapodState, params, 0);
    ...
    std::vector<MDT::RobotState> getNextMCTSstateList_underConstrains_moreStates(const MDT::RobotState rState, const Parameters &params)
    {
        ...
        auto result = PLANNING::getSupportListAndStepL_underConstrains(rState, params);
        std::vector<MDT::Vector6b> availableSupportState_ = result.first;
        ...
        MDT::RobotState hexapodState = rState;
        hexapodState.gaitToNow << MDT::SWING_FLAG, MDT::SWING_FLAG, MDT::SWING_FLAG, MDT::SWING_FLAG, MDT::SWING_FLAG, MDT::SWING_FLAG;

        float stepLength_ = params.norminalFootStepLengthInWorkspace;

        hexapodState.pose.x += stepLength_ * cos(hexapodState.moveDirection);
        hexapodState.pose.y += stepLength_ * sin(hexapodState.moveDirection);

        auto rollAndPitch = getTargetRollAndPitch(hexapodState, params);

        // pitch方向的移动
        hexapodState.pose.pitch = rollAndPitch.second * 3.0f / 4.0f;
        // hexapodState.pose.pitch = pitchMove[i];
        // hexapodState.maxNormalForce[0] = pitchMove[i];

        // keep 0 roll angle
        hexapodState.pose.roll = rollAndPitch.first;

        // z轴方向的移动
        float maxZ = averageFootholdsHeight + params.norminalTrunkHeight;

        hexapodState.pose.z = maxZ;

        // save it
        MDT::Pose NextStatePose = hexapodState.pose;

        auto results = swingLegFoot_position_lists(hexapodState, params, params.norminalFootStepLengthInWorkspace);
        ...
        for (int i = 0; i < availableSupportState_.size(); i++)
        {
            ...
            // 打补丁； 判断机体移动后支撑腿是否超过工作空间
            int ignoreFlag = false;
            for (int legIndex = 0; legIndex < 6; ++legIndex)
            {
                if (hexapodState.gaitToNow[legIndex] == MDT::SUPPORT_FLAG)
                {
                    Eigen::Vector3d footW;
                    footW << hexapodState.feetPosition[legIndex].x(), hexapodState.feetPosition[legIndex].y(), hexapodState.feetPosition[legIndex].z();
                    auto Ab = Robot_State_Transition::get_oneLegKinematics_con_cog_foot(hexapodState.pose, legIndex, false, params);
                    if (!Robot_State_Transition::isInConvex(Ab.first, Ab.second, footW))
                    {
            ...
            {
                state_.gaitToNow = availableSupportState_[i];
                // keep the feet position of support leg
                for (int i = 0; i < 6; ++i)
                {
                    if (state_.gaitToNow(i) == MDT::SUPPORT_FLAG)
                    {
                        state_.feetPosition[i] = rState.feetPosition[i];
                    }
                    // if (state_.gaitToNow(i) == MDT::SWING_FLAG)
                    // {
                    //     float deltaX = state_.feetPosition[i].x() - rState.feetPosition[i].x();
                    //     float deltaY = state_.feetPosition[i].y() - rState.feetPosition[i].y();
                    //     if (sqrt(deltaX * deltaX + deltaY * deltaY) < 0.04)
                    //     {
                    //         state_.feetPosition[i] = rState.feetPosition[i];
                    //         state_.gaitToNow(i) = MDT::SUPPORT_FLAG;
                    //     }
                    // }
                }
                ...
                if (params.COLLISION_CHECK)
                {
                    ...
                    for (int i = 0; i < 6; ++i)
                    {
                        if (state_.gaitToNow(i) == MDT::SWING_FLAG)
                        {
                            ...
                            bool flag_ = COLLISION_CHECK::isFootTrajectoryCollision(startP, endP, params.mapData);
                            if (!flag_)
                            {
                                collisionFlag = true;
                                break;
    ...
    std::vector<MDT::RobotState> getNextMCTSstateByExpert_rotate_moreState(const MDT::RobotState rState, const Parameters &params)
    {
        ...
        for (int i = 0; i < availableSupportState_.size(); i++)
        {
            ...
            auto results = swingLegFoot_position_lists(hexapodState, params, 0.0);
            for (auto state_ : results)
            {
                resultLists.push_back(state_);
            }
    ...
    MDT::RobotState getNextStateByExpert_underConstrain(const MDT::RobotState rState, const Parameters &params)
    {
        ...
        std::cout << "************ averageFootholdsHeight : " << averageFootholdsHeight << std::endl;
        std::cout << "************ hexapodState.pose.z : " << hexapodState.pose.z << std::endl;
        ...
        //     return hexapodState;
        // }
        // grid_map::SignedDistanceField sdf(mapData, params.elevationLayerName, -1, 3);
        ...
        std::vector<MDT::RobotState> resultLists = swingLegFoot_position_lists(hexapodState, params, params.norminalFootStepLengthInWorkspace);
    ...
    {
        static bool gaitFlag = true;
        MDT::Vector6b supportState;

        if (gaitFlag)
        {
            supportState << MDT::SWING_FLAG, MDT::SUPPORT_FLAG, MDT::SWING_FLAG, MDT::SUPPORT_FLAG, MDT::SWING_FLAG, MDT::SUPPORT_FLAG;
            gaitFlag = false;
        }
        else
        {
            supportState << MDT::SUPPORT_FLAG, MDT::SWING_FLAG, MDT::SUPPORT_FLAG, MDT::SWING_FLAG, MDT::SUPPORT_FLAG, MDT::SWING_FLAG;
            gaitFlag = true;
        }

        float stepLength_ = stepLength;
        ...
        std::vector<MDT::RobotState> resultLists = swingLegFoot_position_lists(hexapodState, params, stepLength);
    ...
    MDT::RobotState getNextMCTSstateByExpert_rotate(const MDT::RobotState rState, const Parameters &params)
    {
        ...
        std::vector<MDT::RobotState> resultLists = swingLegFoot_position_lists(hexapodState, params, 0.0);
    ...
    Eigen::Vector3f getTrackMovingDirection(Eigen::Vector3f robotPos, const std::vector<Eigen::Vector3f> &pathPnts)
    {
        ...
        if (index_ == pathPnts.size() - 1)
        {
            ...
            // std::cout << "\033[31m" << "即将超出路径点范围, 机器人将沿着上一个方向继续前进, 超出后MCTS会出错, 请注意！" << "\033[0m" << std::endl;
    ...
    std::pair<double, double> getMaxYawRotationAngle(MDT::RobotState currentState, const Parameters &params)
    {
        ...
        ppose.pitch = 0;
        ppose.yaw = 0;
        Robot_State_Transition::get_kinematics_con_cog_foot(ppose, A_output, b_output, true, params);
    ...
    std::pair<float, float> getTargetYawAndMoveDir(MDT::RobotState currentState, const std::vector<Eigen::Vector3f> &pathPnts, const Parameters &params)
    {
        ...
        auto yawAngleRange = getMaxYawRotationAngle(currentState, params);
