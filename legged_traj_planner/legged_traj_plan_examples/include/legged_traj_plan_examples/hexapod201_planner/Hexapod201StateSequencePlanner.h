  ...
  void nav_callback(const geometry_msgs::PoseStamped &msg) {
    ...
    for (size_t i = 1; i < path2d.size(); i++) {
      ...
      for (int s = 1; s <= num_steps; ++s) {
        ...
        legged_traj_plan::hexapod_State current_state =
            getCurrentHexapodState();
        legged_traj_plan::hexapod_State next_state =
            generateNextTripodState(current_state, step_cmd_vel);
        // legged_traj_plan::hexapod_State next_state =
        // generateNextTripodState(current_state, geometry_msgs::Twist());
