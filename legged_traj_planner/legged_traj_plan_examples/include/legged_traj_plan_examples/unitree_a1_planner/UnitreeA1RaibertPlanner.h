/**
 * @file UnitreeA1RaibertPlanner.h
...
#include "legged_traj_plan_examples/unitree_a1_planner/UnitreeA1PlannerBase.h"
...
struct A1RobotProfile
{
    ...
    std::array<double, 4> foot_end_sdf;
    std::array<bool, 4> support_state;
    geometry_msgs::Twist cmd_vel;
};

struct UnitreeA1RaibertPlannerConfig
{
    int rosRate;
    ...
    void loadParams(ros::NodeHandle &nh, std::string ns = "RaibertPlanner")
    {
        ...
        if (!check_digit)
        {
            ROS_ERROR("Failed to load UnitreeA1RaibertPlannerConfig.");
        }
    }
};

class UnitreeA1RaibertPlanner : public UnitreeA1PlannerBase
{
private:
    ros::Rate rate_;

    UnitreeA1RaibertPlannerConfig config_;
...
public:
                                                 ...
                                                 robot_interface_)
    {
        ...
        cmd_sub_ = nh_.subscribe("/cmd_vel", 1, &UnitreeA1RaibertPlanner::cmd_callback, this);

        timer_ = nh_.createTimer(ros::Duration(0.01), &UnitreeA1RaibertPlanner::timer_callback, this);
