...
class ElSpiderAirPlanner_Legacy
{
private:
    ros::NodeHandle nh_;
    ros::Subscriber sub_;
...
public:
    ElSpiderAirPlanner_Legacy() : robot_interface_(nh_.param("robot_description", std::string(""))),
                                  gridmap_interface_("/grid_map"), whole_body_planner_(gridmap_interface_, robot_interface_),
                                  rate_(20)
    {
        sub_ = nh_.subscribe("/supportStateTopic", 100, &ElSpiderAirPlanner_Legacy::callback, this);
    }
...
int main(int argc, char **argv)
{
    ...
    ElSpiderAirPlanner_Legacy planner;
    planner.run();
    return 0;
}
