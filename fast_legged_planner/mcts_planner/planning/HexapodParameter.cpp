...
namespace HexapodParameter
{
 ...
 // 初始化固定的坐标变换FixJi
    ...
    MDT::POINT init_defaultFoothold(const int &num)
    {
        ...
        initFeetPosition_B_Frame.row(0) << 0.35, -0.23-0.15, -0.14-0.1; // RF
        initFeetPosition_B_Frame.row(1) << 0.05, -0.29-0.15, -0.14-0.1; // RM
        initFeetPosition_B_Frame.row(2) << -0.35, -0.23-0.15, -0.14-0.1;// RB
        initFeetPosition_B_Frame.row(3) << 0.35,  0.23+0.15, -0.14-0.1; // LF
        initFeetPosition_B_Frame.row(4) << 0.05,  0.29+0.15, -0.14-0.1; // LM
        initFeetPosition_B_Frame.row(5) << -0.35,  0.23+0.15, -0.14-0.1;// LB
        MDT::POINT p;
