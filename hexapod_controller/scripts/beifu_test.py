        ...
        while cur_step < len(trajectory.poses):
            ...
            if cur_beifu_Cmd["FG"] == CtrlCmd.IDLE:
                ...
                if self.calPosDisDiff2d(aim_pose, cur_pose) <= self.admit_pose_limit:
                    # 说明已经到达了当前点
                    print(
                        f"[follow_traj]has arrived aim_pose x: {aim_pose.position.x}, y: {aim_pose.position.y}, z: {aim_pose.position.z}")
                    cur_step += 1
                else:
                    print(
                        f"[follow_traj]begin move to aim_pose x: {aim_pose.position.x}, y: {aim_pose.position.y}, z: {aim_pose.position.z}")
