        ...
        try:
            while self.count < max_iterations:
                if op_step == 0:  # 使能
                    ...
                    self.cmdPose["X"] = 150.0         # TA+TM时间()机体前+后-向运动距离 (mm)
                    ...
                    self.cmdPose["FG"] = 0          # PT_MOV_STP-0(波浪运动-指令间减速停止-摆动时间为TA+TM),PT_MOV_BLD-1(持续运动-指令间不减速-摆动时间为TM)
                    self.cmdPose["Res"] = 0         # PT_MOV_REF-0为弹性指令(自动计算步长),PT_MOV_RGD-1为硬性指令(不调整步长)
                    # print("PTCmd", self.ptcmd)
