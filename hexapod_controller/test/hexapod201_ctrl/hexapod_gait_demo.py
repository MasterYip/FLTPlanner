...
class HexapodGaitDemo:
    ...
    def __init__(self, use_dummy: bool = True, plc_ip: str = "10.1.180.190.1.1"):
        """
        Initialize the demo
    ...
    def print_status(self):
        ...
        rospy.loginfo(
            f"Current Position: ({current_pose.position.x:.3f}, {current_pose.position.y:.3f}, {current_pose.position.z:.3f})")
        rospy.loginfo(
            f"Target Position: ({target_pose.position.x:.3f}, {target_pose.position.y:.3f}, {target_pose.position.z:.3f})")
...
def main():
    ...
    plc_ip = str(rospy.get_param('~plc_ip', '10.1.180.190.1.1'))
